...
const { Option } = Select;
const DashboardPage = () => {
    const { user, smsNumbers, services, countries, stats, loading, createSmsNumber, refreshSmsNumber, cancelSmsNumber, refreshData } = useApp();
    ...
    const columns = [
        {
            ...
            render: (service) => {
                ...
                return (serviceData === null || serviceData === void 0 ? void 0 : serviceData.name) || service;
            }
        },
        {
            title: 'País',
            ...
            render: (country) => {
                ...
                return (countryData === null || countryData === void 0 ? void 0 : countryData.name) || country;
            }
        },
        {
            title: 'Número',
        ...
        },
        {
          ...
          {record.status === 'received' && (<Tooltip title="Ver código">
              <Button type="text" icon={<EyeOutlined />} onClick={() => message.info(`Código: ${record.code}`)} size="small"/>
            </Tooltip>)}
        </Space>)
        }
    ];
      ...
      <div className={styles.container}>
        ...
        <Row gutter={[24, 24]} className={styles.statsRow}>
          <Col xs={24} sm={12} lg={6}>
            <Card>
              <Statistic title="Créditos Disponíveis" value={(user === null || user === void 0 ? void 0 : user.credits) || 0} prefix={<CreditCardOutlined style={{ color: '#52c41a' }}/>} valueStyle={{ color: '#52c41a' }}/>
            </Card>
          </Col>
          <Col xs={24} sm={12} lg={6}>
            <Card>
              <Statistic title="Usados Hoje" value={stats.usedToday} prefix={<MessageOutlined style={{ color: '#1677ff' }}/>} valueStyle={{ color: '#1677ff' }}/>
            </Card>
          </Col>
          <Col xs={24} sm={12} lg={6}>
            <Card>
        ...
        {/* Números SMS */}
        <Card title="Números SMS" extra={<Space>
              <Button icon={<ReloadOutlined />} onClick={refreshData} loading={loading.smsNumbers}>
                Atualizar
              </Button>
              <Button type="primary" icon={<PlusOutlined />} onClick={() => setIsModalVisible(true)} loading={loading.newNumber}>
                Novo Número
              </Button>
            </Space>} className={styles.numbersCard}>
          <Text type="secondary" style={{ marginBottom: 16, display: 'block' }}>
            Gerencie seus números virtuais e códigos recebidos
          </Text>
          
          <Table columns={columns} dataSource={smsNumbers} rowKey="id" loading={loading.smsNumbers} pagination={{
            pageSize: 10,
            showSizeChanger: true,
            showQuickJumper: true,
            showTotal: (total, range) => `${range[0]}-${range[1]} de ${total} números`
        }} scroll={{ x: 800 }}/>
        </Card>

        {/* Modal para novo número */}
        <Modal title="Solicitar Novo Número" open={isModalVisible} onCancel={() => {
            setIsModalVisible(false);
            form.resetFields();
          ...
          <Form form={form} layout="vertical" onFinish={handleCreateNumber} style={{ marginTop: 24 }}>
            <Form.Item name="service" label="Serviço" rules={[{ required: true, message: 'Selecione um serviço' }]}>
              <Select placeholder="Escolha o serviço" size="large" showSearch filterOption={(input, option) => {
            var _a, _b;
            return (_b = (_a = option === null || option === void 0 ? void 0 : option.children) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === null || _b === void 0 ? void 0 : _b.includes(input.toLowerCase());
        }}>
            ...
            <Form.Item name="country" label="País" rules={[{ required: true, message: 'Selecione um país' }]}>
              <Select placeholder="Escolha o país" size="large" showSearch filterOption={(input, option) => {
            var _a, _b;
            return (_b = (_a = option === null || option === void 0 ? void 0 : option.children) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === null || _b === void 0 ? void 0 : _b.includes(input.toLowerCase());
        }}>
