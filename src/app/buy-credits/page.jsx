...
const BuyCreditsPage = () => {
    ...
    const handlePayment = async (values) => {
        try {
            // Nota: a lógica interna do `purchaseCredits` no AppContext foi alterada,
            // mas a chamada a partir daqui permanece a mesma.
    ...
    if (loading.auth && !creditPackages.length) { // Melhorado para evitar piscar a tela
        return (
                    ...
                    <div className={styles.summaryContent}>
                        ...
                        {paymentMethod === 'pix' && (
                            <div className={styles.summaryItem}>
                                ...
                                <Text type="success">-R$ {(selectedPkg.price * 0.05).toFixed(2)}</Text>
                            </div>
                        )}
                    ...
                    {paymentMethod === 'pix' ? (
                        <div style={{ textAlign: 'center', padding: '24px 0' }}>
                            ...
                            <div className={styles.modalSummary}>
                                ...
                                <Text>Pacote {selectedPkg.name}</Text>
                                <Text strong>R$ {getPackagePrice(selectedPkg).toFixed(2)}</Text>
                                </div>
                                <Text type="secondary">
                                    {selectedPkg.credits.toLocaleString()} créditos
                                    {selectedPkg.bonus && ` + ${selectedPkg.bonus} bônus`}
                        ...
                        <Form form={form} layout="vertical" onFinish={handlePayment} style={{ marginTop: 24 }}>
                            <Form.Item name="cardNumber" label="Número do Cartão" rules={[{ required: true, message: 'Digite o número do cartão' }, { pattern: /^\d{16}$/, message: 'Digite um número válido (16 dígitos)' }]}>
                                ...
                                <Col span={8}>
                                    <Form.Item name="expiryYear" label="Ano" rules={[{ required: true, message: 'Ano' }]}>
                                        <Select placeholder="AAAA" size="large">
                                            {Array.from({ length: 10 }, (_, i) => { const year = new Date().getFullYear() + i; return (<Option key={year} value={String(year)}>{year}</Option>);})}
                                        </Select>
                                    </Form.Item>
                                </Col>
                                <Col span={8}>
                                    <Form.Item name="cvv" label="CVV" rules={[{ required: true, message: 'CVV' }, { pattern: /^\d{3,4}$/, message: 'CVV inválido' }]}>
                            ...
                            <div style={{ textAlign: 'right', marginTop: 24 }}>
                                ...
                                <Button type="primary" htmlType="submit" loading={loading.payment} size="large">Confirmar Pagamento</Button>
                                </Space>
                            </div>
                        </Form>
                    )}
